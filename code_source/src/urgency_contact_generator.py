...
def generate_urgency_contacts_sheet(participants_data):
    ...
    active_participants.sort(key=lambda x: (
        str(x.get("user_lastName", "")).strip().lower(),
        str(x.get("user_firstName", "")).strip().lower()
    ))
    ...
    for p in active_participants:
        ...
        group_name = str(p.get("tarif_name", "")).strip()
        
        # Concaténer les deux contacts d'urgence (avec formatage NOM et Prénom)
        c1_name_raw = str(p.get("champ_Personne à prévenir en cas d'urgence - NOM  et PRENOM en majuscule", "")).strip()
        c1_name = format_contact_name(c1_name_raw)
        c1_tel = str(p.get("champ_Personne à prévenir en cas d'urgence - Téléphone", "")).strip()
        
        c2_name_raw = str(p.get("champ_Parent 2  à prévenir en cas d'urgence - NOM ET PRENOM (en majuscule)", "")).strip()
        c2_name = format_contact_name(c2_name_raw)
        c2_tel = str(p.get("champ_Parent 2 - Numéro de téléphone portable", "")).strip()
